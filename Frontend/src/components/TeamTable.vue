...
const createTeam = async () => {
  ...
  try {
    const res = await axios.post('https://sports-scheduling-application.onrender.com/api/teams', payload)
    if (res.status === 201) {
...
const saveEdit = async (originalTeam) => {
  ...
  try {
    const res = await axios.put(`https://sports-scheduling-application.onrender.com/api/teams/${originalTeam._id}`, updatedTeam)
    if (res.status === 200) {
